...
#import <ios/Classes/GKAchievementNotification.h>
...
/**
 * Показать ачивмент с кастомным заголовком и без подзаголовка.
 * @param title  Заголовок ачивмента.
 */
- (void)notifyAchievementTitle:(NSString *)title;

/**
 * Показать ачивмент с кастомным заголовком и кастомной картинкой
 * @param title  Заголовок ачивмента.
 * @param image  Иконка ачивмента
 */
- (void)notifyAchievementTitle:(NSString *)title andImage:(UIImage *)image;

/**
 * Вернуть путь к графическим ресурсам
 */
+ (NSString*)pathToGraphics;
